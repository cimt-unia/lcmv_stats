...
Combines:
  - Feature extraction (WPLI, GPDC) via flexible ROI selection
...
def compute_connectivity_matrix(
    ...
    """
    Compute symmetric connectivity matrix using MNE multitaper.
    Pure computational engine. No atlas awareness.
    ...
    Args:
        ...
        method: Connectivity metric (default: wpli2_debiased).

    Returns:
        Symmetric (n_signals, n_signals) numpy array with zero diagonal.
    """
...
# =============================================================================
# 1. FEATURE EXTRACTION (FLEXIBLE ROI SELECTION)
# =============================================================================
...
def extract_wpli_features(
    ...
    """
    Compute WPLI for an arbitrary ROI subset on two conditions.

    Provide EITHER explicit roi_names OR metadata filters (functional_systems,
    sub_systems, hemisphere). If neither is provided, defaults to the full
    Motor-Basal-Executive-STN network.

    Args:
        epochs_task_A: (n_epochs, n_rois_full, n_times) — Condition A epochs.
        epochs_task_B: Same shape — Condition B epochs.
        roi_names: Explicit list of ROI names.
        functional_systems: e.g., ['Motor', 'BasalGanglia', 'Frontoparietal']
        sub_systems: e.g., ['Primary', 'Premotor', 'Subthalamic']
        hemisphere: 'Left' or 'Right'
        band: Frequency band name.
        sfreq: Sampling frequency.

    Returns:
        Tuple of (conn_matrix_A, conn_matrix_B, roi_indices_used) or (None, None, empty).
    """
    ...
    if band not in bands:
        raise ValueError(f"Unknown band '{band}'. Available: {list(bands.keys())}")
    fmin, fmax = bands[band]

    try:
        # Resolve ROI indices using flexible selection
        if roi_names is not None:
            indices = resolve_roi_indices(roi_names)
        elif functional_systems is not None or sub_systems is not None or hemisphere is not None:
            indices = select_network(
        ...
        if len(indices) == 0:
            logger.warning("ROI selection returned 0 ROIs.")
            return None, None, indices

        # Slice and compute
        data_a = epochs_task_A[:, indices, :]
        data_b = epochs_task_B[:, indices, :]

        conn_a = compute_connectivity_matrix(data_a, fmin, fmax, sfreq)
        conn_b = compute_connectivity_matrix(data_b, fmin, fmax, sfreq)
        return conn_a, conn_b, indices
...
def extract_gpdc_features(
    ...
    Args:
        epochs_task_A/B: Shape (n_epochs, n_rois_full, n_times).
...
def run_edgewise_permutation(
    ...
    Args:
        data_a: (n_subjects, n_edges) — Condition A.
        data_b: (n_subjects, n_edges) — Condition B.
        roi_names: Array of ROI names used to generate these edges.
    ...
    # Build edge index pairs from the actual number of ROIs
...
def compare_tensors(
    ...
    for i, sid in enumerate(tens_a['subject_ids']):
        ...
        if conn_a is not None and conn_b is not None:
            ...
            if roi_indices_used is None:
                roi_indices_used = indices
    ...
    selected_roi_names = atlas_df.loc[atlas_df['index'].isin(roi_indices_used), 'roi_name'].values
...
def save_comparison_results(result: Dict, output_path: str | Path):
    ...
    np.savez_compressed(
        ...
        metadata_n_rois=result['metadata']['n_rois']
    )
